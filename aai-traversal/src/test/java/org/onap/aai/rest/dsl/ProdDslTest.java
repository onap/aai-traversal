...
//TODO: Add queries run by SEs in narad-prod for dbpatch
...
public class ProdDslTest extends AAISetup {
    ...
    public void msoQueryTest2() throws AAIException {
        String aaiQuery = "pserver('hostname', 'rdm52r19c001.region2.cci.att.com') > p-interface > sriov-pf*('pf-pci-id', '0000:5e:01.0')";

        String dslQuery = "builder.getVerticesByProperty('aai-node-type', 'pserver').getVerticesByProperty('hostname','rdm52r19c001.region2.cci.att.com')"
                + ".createEdgeTraversal(EdgeType.TREE, 'pserver','p-interface')"
                + ".createEdgeTraversal(EdgeType.TREE, 'p-interface','sriov-pf').getVerticesByProperty('pf-pci-id','0000:5e:01.0').store('x').cap('x').unfold().dedup()";
    ...
    //TODO : Get this from narad schema
    @Test

...
if [ -z ${DISABLE_UPDATE_QUERY} ]; then
    ...
    AAICONFIG_FILE=/opt/app/aai-traversal/resources/etc/appprops/aaiconfig.properties;
    ...
    if [ ! -f ${UPDATE_QUERY_RAN_FILE} ]; then
            ...
            gosu aaiadmin sed -i "s%^aai.server.url=.*$%aai.server.url=${TEMP_RESOURCES_URL}%g" ${AAICONFIG_FILE};
            gosu aaiadmin /opt/app/aai-traversal/bin/install/updateQueryData.sh
            touch ${UPDATE_QUERY_RAN_FILE};
            gosu aaiadmin sed -i "s%^aai.server.url=.*$%aai.server.url=${OLD_RESOURCES_URL}%g" ${AAICONFIG_FILE};
    fi
fi
